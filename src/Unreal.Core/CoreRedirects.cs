...
namespace Unreal.Core
{
    ...
    public static class CoreRedirects
    {
        ...
        //Generates possible exports for a specific path
        private static string FindPath(string path)
        {
            foreach(string nativePath in NativeRedirects.Keys)
            {
                if(path.StartsWith(nativePath))
                {
                    NativeRedirects[path] = NativeRedirects[nativePath];

                    return NativeRedirects[path];
                }
            }
        ...
        public static string GenerateCSharp()
        {
            ...
            builder.AppendLine("private static readonly Dictionary<string, string> NativeRedirects = new Dictionary<string, string>");
            builder.AppendLine("{");

            foreach(var kvp in NativeRedirects)
            {
